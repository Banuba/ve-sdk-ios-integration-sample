...
final class VideoEditorModule: BanubaVideoEditorDelegate {
  ...
  let onVideoEditingFinished: (Result<VideoEditingResult, Error>) -> Void
  ...
  init?(
    ...
    onVideoEditingFinished: @escaping (Result<VideoEditingResult, Error>) -> Void
  ) {
  ...
  private func export(_ completion: @escaping (Result<VideoEditingResult, Error>) -> Void) {
    // Show progress VC
